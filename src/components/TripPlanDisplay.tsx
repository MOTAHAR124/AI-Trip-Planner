...
export default function TripPlanDisplay({ tripPlan }: TripPlanDisplayProps) {
  ...
  useEffect(() => {
    return () => {
      ...
      if (downloadedTimeoutRef.current) window.clearTimeout(downloadedTimeoutRef.current);
    };
  }, []);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 sm:mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2 print:hidden">
            ...
            >
              ...
              <span className="sm:hidden text-xs">{copied ? 'Copied' : 'Copy'}</span>
            </button>
            <button
              onClick={() => {
                ...
                setDownloaded(true);
                if (downloadedTimeoutRef.current) window.clearTimeout(downloadedTimeoutRef.current);
                downloadedTimeoutRef.current = window.setTimeout(() => setDownloaded(false), 2000);
              }}
              className="flex items-center space-x-2 px-3 sm:px-4 py-2 text-gray-600 hover:text-gray-800 hover:bg-gray-50 rounded-lg transition-colors border border-gray-200 cursor-pointer text-sm"
            ...
            >
              ...
              <span className="hidden sm:inline text-sm font-medium">{downloaded ? 'Downloaded' : 'Download'}</span>
              <span className="sm:hidden text-xs">{downloaded ? 'Done' : 'DL'}</span>
            </button>
            <button
